<script lang="ts">
    ...
    let x = $state(0), y = $state(0), size = $state(0);
    let show = $state(false), back = delayedToggle(false, 0), active = delayedToggle(_active, 0);
    let render = delayedToggle(false, duration, duration);
    let adapter, startTs = 0;
    let iv = 0, run = 0;

    let container = $derived(extra || adapter?.parentElement)

    $effect(() => active.v = _active)
    $effect(() => {
        render.v = show
    })
    ...
    const rippleSize = (targetX: number, targetY: number) => {
        ...
        const w = r.width, h = r.height, t = r.top, l = r.left
        if (!center) {
            const x = targetX - l - window.scrollX, y = targetY - t - window.scrollY
            const _x = Math.max(x, w - x), _y = Math.max(y, h - y)
            const size = Math.sqrt(_x * _x + _y * _y) * 2
            return {x, y, size}
        } else return {x: w / 2, y: h / 2, size: Math.max(w, h)}
    }

    const rippleShowEvent = (targetX, targetY) => {
        if (run) return run = 1
        run = 1
    ...
    const showBackground = (e) => back.v = true
...
<style lang="scss">
  * {
  ...
  div {
    ...
    animation: scaleIn var(--dur, 600ms) cubic-bezier(0, .57, .1, .98) forwards;
  }
